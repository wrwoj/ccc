%{
   ...
   * available from AT&T.
   *
   * This program wash students the use of bison and finite-state automata.
   * Modified in 2008 and in 2011, in 2015, and in 2019 by Jan Daciuk.
   *
   * Rather minor changes here were introduced by Wiktor Wojtyna
    as a part of his task for the Compiler Construction course that was taught by the aformentioned Jan Daciuk.
   */
  #include	<stdio.h>
  #include	<string.h>
  #include	<stdlib.h>
...
RE: RE '|' RE {
  int s1, s2;
  s1 = create_state(); s2 = create_state();
  create_transition(s1, FIRST($3), EPSILON);
  create_transition(s1, FIRST($1), EPSILON);
  create_transition(LAST($3),s2, EPSILON);
  create_transition(LAST($1),s2, EPSILON);
  $$ = createRE(s1, s2);
}
;
/* Kleene's star */
RE: RE '*' {
  ...
  s1 = create_state(); s2 = create_state();
  create_transition(s1, FIRST($1), EPSILON);
  create_transition(s1, s2, EPSILON);
