...
%type <s> FUN_HEAD FUN_CALL VAR BLOCK EXPR COND_EXPR
...
/* DECL_LIST */
/* Declaration list can be empty or it may consist of a sequence of declarations (S_DECLARATION) */
DECL_LIST:
      %empty
...
/* FUN_CALL */
 /* function call consists of an identifier, left parenthesis,
...
/* ACT_PARAMS */
/* Actual parameters can be empty or a list of actual parameters (ACT_PARAM_LIST) */
ACT_PARAMS: %empty
          | ACT_PARAM_LIST;

/* ACT_PARAM_LIST */
/* List of actual parameters is a non-empty, comma-separated list */
ACT_PARAM_LIST: ACT_PARAM
              | ACT_PARAM_LIST ',' ACT_PARAM;

/* ACT_PARAM */
/* Actual parameter can be an expression (EXPR) or a string (STRING_CONST) */
ACT_PARAM: EXPR { found("ACT_PARAM", "EXPR"); }
         | STRING_CONST { found("ACT_PARAM", $1); };
...
/* ELSE_PART */
/* else part can either be empty or it may consist of keyword else (KW_ELSE)
   and a block (BLOCK) */
...
/* do while loop consists of keyword do (KW_DO), block (BLOCK),
   keyword while (KW_WHILE), left parenthesis,
...
COND_EXPR: LOG_EXPR '?' EXPR ':' EXPR { found("COND_EXPR", ""); }
         | LOG_EXPR '?' COND_EXPR ':' EXPR { found("COND_EXPR", ""); }
         | LOG_EXPR '?' EXPR ':' COND_EXPR { found("COND_EXPR", ""); }
         | LOG_EXPR '?' COND_EXPR ':' COND_EXPR { found("COND_EXPR", ""); };
%%

int main(void)
{
	int ret;
	printf("Author: Wiktor Wojtyna\n");
